...
class TestFilterBySkills:
    ...
    def test_multiple_skills_any_match(self):
        # c3 only has Python (not Django) but still matches since only one
        # required skill needs to overlap.
        result = filter_by_skills(CANDIDATES, ["Python", "Django"])
        ids = [c["candidate_id"] for c in result]
        assert set(ids) == {"c1", "c3", "c4"}

    def test_partial_normalized_match(self):
        # "Gen AI" (typed) should match a parsed skill of "GenAI" (no space).
        candidates = [_make_candidate("cx", ["GenAI", "Docker"], "Remote", 2)]
        result = filter_by_skills(candidates, ["Gen AI"])
        assert len(result) == 1
