...
from utils.utils import get_logger, extract_llm_text
...
class ReRanker:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.llm = ChatGoogleGenerativeAI(
                ...
                max_retries=1,  # fail fast to the rule-based fallback instead of long backoffs
            )
    ...
    def evaluate_candidate(self, resume: Resume, jd: SearchQuery) -> CandidateEvaluation:
        ...
        try:
            ...
            txt = extract_llm_text(resp.content)
    ...
    def extract_section(self, text: str, section_name: str, max_items: int) -> List[str]:
        """Parse an LLM response and return list items for a named section.

        Strategy:
        - Find the `section_name:` header and capture everything up to the next
          known header (or end of text), regardless of whether the model put
          each item on its own line or inline after the header on one line —
          both are common depending on the model/prompt.
        - Split that captured text on numbered ("1.", "2)") or bullet (-, •, *)
          markers to recover individual items.
        - Original casing is preserved in the returned items.
        """
        all_headers = ['strengths', 'gaps', 'risks', 'summary']
        other_headers = [h for h in all_headers if h != section_name]
        stop_pattern = '|'.join(other_headers) if other_headers else r'\Z'
        match = re.search(
            rf'{re.escape(section_name)}\s*:\s*(.*?)(?=(?:{stop_pattern})\s*:|\Z)',
            text,
        ...
        # Split on numbered/bulleted markers, keeping items in original order
