...
def load_resumes(directory: str, parse: bool = True) -> list[Document]:
    """Load all resume PDFs from directory and return Document objects.

    When `parse` is True (default), each resume is also parsed with Gemini for
    rich metadata (skills, location, experience). Pass `parse=False` to skip
    the LLM entirely and get text-only Documents with fallback metadata —
    used when rebuilding the in-memory BM25 index from resumes that are
    already fully indexed in Pinecone, so no redundant Gemini calls are made.
    """
    resumes = []
    ...
    for file in os.listdir(directory):
        ...
        if file.lower().endswith('.pdf'):
            try:
                ...
                parsed = _try_parse_resume(text, candidate_id) if parse else {}
