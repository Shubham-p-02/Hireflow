        ...
        try:
            ...
            if hybrid_indexer and vector_store_ready:
                try:
                    ...
                    # Skip Gemini parsing when Pinecone already has the real
                    # metadata — this rebuild only needs raw text for BM25.
                    existing_resumes = load_resumes(str(DATA_RESUMES_DIR), parse=not already_indexed)
                    if existing_resumes:
                        if already_indexed:
