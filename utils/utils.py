...
def extract_llm_text(content) -> str:
    """Normalize a chat model's `.content` to plain text.

    Depending on the model/SDK version, langchain-google-genai returns
    `.content` as either a plain string or a list of content-block dicts
    (e.g. [{'type': 'text', 'text': '...'}]). Callers that expect a string
    (for .split(), .strip(), etc.) should route through this first.
    """
